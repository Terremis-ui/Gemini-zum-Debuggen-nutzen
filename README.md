# Gemini-zum-Debuggen-nutzen "Arch Linux"
1. API Key erstellen auf Google AI Studio
2. export GEMINI_API_KEY="AIzaSy..." -  "Dein API key in die Ausrufezeichen einfügen" oder die py bearbeiten Zeile
if __name__ == "__main__":
 key = os.getenv("GEMINI_API_KEY")
3. Nutzen zb # konsole 2>&1 | python3 ~/Downloads/gemini_bootstrap.py


Die py ist auf Arch Linux angepasst lässt sich aber auf jede beliebige Distribution umschreben #Diese zeile anpassen# system_instruction = (
        "Du bist der Terremis-Assistent. Analysiere diesen Pacman-, KDE- oder Code-Fehler "
        "und gib eine prägnante, verständliche Lösung für Arch Linux aus."

Nicht über den Namen wundern das war ursprünglich eine Idee um von der API optimale Gentoo Installationsanweisungen zu bekommen, sie ist also live ISO getestet.
        
Ich hoffe ich hab nichts ausgelassen mache das zum ersten mal xD
